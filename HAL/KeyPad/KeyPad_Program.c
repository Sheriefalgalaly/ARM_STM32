...
u8 KeyPad_voidRead(KeyPad_t* key)
{
    ...
    {
    	    GPIO_PINWrite_High(*(key->PortColumn+Local_u8ColumnHigh),*(key->PinColumn+Local_u8ColumnHigh));
    		for (u8 Local_u8Column=0;Local_u8Column<NumOfColumns;Local_u8Column++)
    		{
    			if (Local_u8ColumnHigh != Local_u8Column )
    		    	  GPIO_PINWrite_LOW(*(key->PortColumn+Local_u8Column),*(key->PinColumn+Local_u8Column));
    		}
    		//read input
